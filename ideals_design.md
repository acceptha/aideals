...
### Step 1 — 카테고리 선택
사용자가 의류 카테고리(아우터, 상의, 하의, 원피스/스커트, 신발, 가방) 6개 중 하나를 선택한다. 세부 카테고리(소분류)는 두지 않는다.

### Step 2 — 셀럽/인플루언서 스타일 탐색
선택한 카테고리에 해당하는 셀럽·인플루언서 착용 사진이 그리드 형태로 노출된다. 필터(성별, 색상)를 통해 결과를 좁힐 수 있다. 시즌은 사용자가 직접 필터링하지 않고, 현재 계절에 맞는 룩북이 자동으로 상위 노출된다.

#### 시즌 자동 정렬
한국 계절 기준으로 현재 월에 해당하는 시즌의 룩북에 정렬 가중치를 부여한다. 다른 시즌의 룩북은 숨기지 않고 하단에 함께 노출한다.
...
환절기(3월, 6월, 9월, 12월)는 이전/다음 시즌도 함께 가중치를 높여 자연스러운 전환을 제공한다.

#### 필터 항목

| 필터 | 값 | 설명 |
|------|----|------|
| 성별 | 남성, 여성 | 단일 선택. 선택 해제 시 전체(남녀 모두) 노출 |
| 색상 | 블랙, 화이트, 베이지, 그레이, 네이비, 브라운, 레드, 블루, 그린, 핑크 등 | 다중 선택 |

### Step 3 — 유사 상품 비교
사진을 선택하면, 해당 아이템과 유사한 여러 브랜드의 상품이 카드 리스트로 표시된다. 각 카드에는 상품 이미지, 브랜드명, 상품명, 가격이 포함되며, 가격순·브랜드순·유사도순 정렬이 가능하다. 가격대 필터(~3만원, 3~5만원, 5~10만원, 10~20만원, 20만원~)를 통해 예산에 맞는 유사 상품만 좁혀서 볼 수 있다.
...
| 모듈 | 역할 |
|------|------|
| **Category Service** | 카테고리 목록 관리 (대분류 6개, 세부 카테고리 없음) |
...
```
GET  /api/styles                  // 스타일 목록 (필터: category, gender, color / 시즌 자동 정렬)
...
|------|-------------------|---------------------|
| 카테고리 | 아이콘 그리드 (3열, 대분류 6개) | 아이콘 그리드 (6열, 대분류 6개) |
...
- **체형 기반 필터**: 사용자가 키/몸무게를 입력하면 유사 체형의 인플루언서 룩북을 우선 노출. 인플루언서 신체 정보 수급이 어려우므로 체형 태그(슬림, 보통, 탄탄 등)로 단순화하는 방안 검토
- **카테고리 세분화**: 대분류 6개에서 필요 시 소분류 추가 (예: 아우터 → 블레이저, 패딩, 코트). 콘텐츠 양이 카테고리당 충분히 쌓인 뒤 도입
