...
```
Category (트리) ──1:N──▶ CelebStyle ──1:N──▶ SimilarProduct ──1:N──▶ PurchaseLink
    │
    └── self-referencing (parentId → id) : 대분류-소분류 트리
```
...
- `gender` 허용 값: `"male"` | `"female"` | `"unisex"`
- `season` 허용 값: `"spring"` | `"summer"` | `"fall"` | `"winter"` | `"all"`
- `tags`는 문자열 배열 (예: `["캐주얼", "스트릿", "미니멀"]`)
- `currency` 기본값: `"KRW"`
