...
1. **카테고리 선택**: 대분류 6개(아우터, 상의, 하의, 원피스/스커트, 신발, 가방) 선택. 세부 카테고리(소분류) 없음
2. **셀럽 스타일 탐색**: 선택 카테고리에 해당하는 착용 사진 그리드 노출. 성별·색상 필터 지원. 시즌은 현재 계절 기준 자동 정렬
3. **유사 상품 비교**: 사진 선택 시 유사 상품 카드 리스트 표시 (이미지, 브랜드, 상품명, 가격). 가격대 필터 + 가격순·브랜드순·유사도순 정렬
...
- **스키마 원본 (Single Source of Truth)**: `prisma/schema.prisma`
- **비즈니스 규칙·테이블 매핑**: `docs/schema.md`
- **핵심 엔티티**: Category, CelebStyle, SimilarProduct, PurchaseLink
- **관계 요약**: Category → CelebStyle 1:N → SimilarProduct 1:N → PurchaseLink 1:N
- DB 관련 작업 시 `prisma/schema.prisma`를 먼저 읽고, 비즈니스 규칙은 `docs/schema.md` 참조
...
| GET | `/api/categories/:id/styles` | 특정 카테고리의 셀럽 스타일 목록 | `gender`, `color`, `page`, `limit` |

### 셀럽 스타일

| 메서드 | 경로 | 설명 | 쿼리 파라미터 |
|--------|------|------|---------------|
| GET | `/api/styles` | 스타일 목록 (필터링) | `categoryId`, `gender`, `color`, `page`, `limit`, `sort` |
...
- 카테고리 아이콘을 그리드로 표시
- 모바일: 3열, 데스크톱: 6열
- 대분류 6개 클릭 시 해당 카테고리의 스타일 목록(`/styles?categoryId=`)으로 이동
...
- 성별(남/여) 단일 선택(선택 해제 시 전체), 색상(블랙, 화이트 등) 다중 선택
...
- 외부 이미지 도메인은 `next.config.mjs`의 `images.remotePatterns`에 등록한다.
...
> 에러 코드 목록, 응답 포맷, 구현 패턴 상세는 **[PROJECT_RULES.md > 5. 에러 코드 체계](./PROJECT_RULES.md#5-에러-코드-체계)** 를 참고한다.
...
export const GET = withErrorHandler(async (req: NextRequest) => {
  ...
  const [styles, total] = await getStylesWithCount({ categoryId: params.categoryId, ... });
...
> API Route는 Prisma를 직접 호출하지 않고, `src/lib/data/` 계층의 함수를 사용한다. 캐싱은 data 계층 내부에서 처리된다.
...
> 에러 클래스, 핸들러 래퍼, 응답 타입, 에러 코드 목록 등 **전체 구현 패턴은 [PROJECT_RULES.md > 5. 에러 코드 체계](./PROJECT_RULES.md#5-에러-코드-체계)** 를 참고한다.
...
현재 프로젝트는 GET 전용 API만 구현되어 있어 Body 파서는 미구현 상태다. POST/PUT/PATCH API 추가 시 `src/lib/api/parseBody.ts`를 구현한다.
...
│   ├── data/                     # 데이터 접근 계층 (캐싱 포함, API·페이지 공용)
...
│   ├── redis.ts                  # Upstash Redis 클라이언트 싱글턴
│   ├── cache.ts                  # 캐시 유틸 (withCache, buildCacheKey, CACHE_TTL)
│   └── scraper/                  # 크롤러 모듈
├── stores/                       # Zustand 스토어
├── types/                        # 공유 타입 정의
│   ├── index.ts                  # 도메인 엔티티 타입
...
- **데이터 접근 함수 (캐싱 포함)**: `src/lib/data/` 아래에 배치
- **순수 함수 (날짜 포맷, 가격 포맷 등)**: 필요 시 `src/utils/`를 생성하여 배치
...
- 이미지 도메인을 추가할 때는 `next.config.mjs`의 `images.remotePatterns`를 업데이트한다.
